...
import { AutoApproveGate, AutoDenyGate, type ApprovalGate } from "./approval.js";
...
if ((args.has("--deny") && args.has("--approve")) || (!args.has("--deny") && !args.has("--approve"))) {
  throw new Error("Pass exactly one of --deny or --approve");
}
...
const brief = BriefSchema.parse({
  ...
  platform: "tiktok",
  mediaPaths: ["fixtures/demo-hook.mp4", "fixtures/demo-proof.mp4", "fixtures/demo-result.mp4"],
});
...
const gate = args.has("--approve") ? new AutoApproveGate() : new AutoDenyGate();
...
const inspectingGate: ApprovalGate = {
  async request(action) {
    ...
    return await gate.request(action);
  },
};
const writeResult = await renderPackage(result.plan, inspectingGate, { trace: demoTrace });
let afterExists = false;
let modifiedAt = "not written";
let afterModifiedAt: number | undefined;
const target = writeResult.rendered ? writeResult.path : writeResult.action.touches[1];
if (target) {
  try {
    await access(target);
    afterExists = true;
...
row("tool", writeResult.action.tool);
row("exact argv", JSON.stringify(writeResult.action.argv));
row("touches", writeResult.action.touches.join(", "));
row("decision", writeResult.rendered ? "APPROVED for this action" : "DENIED");
row("write performed", writeResult.rendered ? "yes" : "no");
if (!writeResult.rendered) row("denial result", `rendered=false, reason=${writeResult.reason}`);
...
if (writeResult.rendered) {
  ...
  row("packet", "not created or changed by this denied action");
